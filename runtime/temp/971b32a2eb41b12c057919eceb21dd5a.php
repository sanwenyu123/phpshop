<?php if (!defined('THINK_PATH')) exit(); /*a:5:{s:38:"./template/pc/default/index\index.html";i:1490353006;s:40:"./template/pc/default/public\header.html";i:1490353006;s:44:"./template/pc/default/public\siteTopbar.html";i:1490353006;s:38:"./template/pc/default/public\kefu.html";i:1490353006;s:40:"./template/pc/default/public\footer.html";i:1490353006;}*/ ?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
<div class="top-banner" id="top-banner-block">
    ...
    <?php $pid =1;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
...
<!--------banner-开始-------------->
	<div class="nav-banner">
        <div id="banner_tabs" class="flexslider">
            ...
            <?php $pid =2;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
    ...
    <div class="layout">
    	<div class="bs-le">
        	<div class="first">
                ...
                <?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods` where is_new = 1 and is_hot = 1 and is_recommend = 1 and is_on_sale = 1 order by sort desc limit 4");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__goods` where is_new = 1 and is_hot = 1 and is_recommend = 1 and is_on_sale = 1 order by sort desc limit 4"); 
                                    S("sql_".$md5_key,$sql_result_v,31104000);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
            		<li class="sellers">
    	...
    	<div class="bs-ri">
        	<div class="ris-adve"><!--公告上方广告位-->
                <?php $pid =7;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    ?>
        	...
        	<div class="ris-notice">
            	...
            	<div class="notice_b">
                	<ul class="nob1">
                            <?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__article`  where cat_id = 4 order by article_id desc limit 4");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__article`  where cat_id = 4 order by article_id desc limit 4"); 
                                    S("sql_".$md5_key,$sql_result_v,31104000);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
                		<li><a href="<?php echo U('Home/Article/detail',array('article_id'=>$v[article_id])); ?>"><?php echo getSubstr($v['title'],0,18); ?></a></li>
                            <?php endforeach; ?>    
                	</ul>
                    <ul class="nob2">
                            <?php
                                   
                                $md5_key = md5("select * from `__PREFIX__article`  where cat_id = 3 order by article_id desc limit 4");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__article`  where cat_id = 3 order by article_id desc limit 4"); 
                                    S("sql_".$md5_key,$sql_result_v,31104000);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
                		<li><a href="<?php echo U('Home/Article/detail',array('article_id'=>$v[article_id])); ?>"><?php echo getSubstr($v['title'],0,18); ?></a></li>
             ...
             <!--公告下方广告位-->
        	 <div class="ris-as"> 
                <?php $pid =8;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
foreach($result as $key=>$v):       
    ...
    ?>
                     <p><a href="<?php echo $v[ad_link]; ?>" <?php if($v['target'] == 1): ?>target="_blank"<?php endif; ?> ><img src="<?php echo $v[ad_code]; ?>" width="278" height="132" title="<?php echo $v[title]; ?>" style="<?php echo $v[style]; ?>"></a></p>                
                <?php endforeach; ?>
            </div>
        	</div>	
        </div>
...
<!--------中部banner-开始-------------->
	<div class="layout">
    	<div class="cen-banne">
            <?php $pid =3;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
...
<?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods_category` where is_show = 1 and `level` = 1  limit 7");
                                $result_name = $sql_result_val = S("sql_".$md5_key);
                                if(empty($sql_result_val))
                                {                            
                                    $result_name = $sql_result_val = \think\Db::query("select * from `__PREFIX__goods_category` where is_show = 1 and `level` = 1  limit 7"); 
    	...
    	<div class="ground-flool">
        	<div class="flool-t">
                ...
                <?php
                                ...
                                {                            
                                    $result_name = $sql_result_val2 = \think\Db::query("select * from `__PREFIX__goods_category` where is_show = 1 and parent_id = $val[id]"); 
                                    S("sql_".$md5_key,$sql_result_val2,31104000);
                                }    
                              foreach($sql_result_val2 as $key2=>$val2): ?>
	                <em><?php echo $val2[name]; ?></em>
                <?php endforeach; ?>                
                <ul><!--
            	...
            	<ul class="flool-cle">
            		<li class="sellers sell_ons">
                        ...
                        <?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by sort desc limit 1");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__goods` where cat_id in($cat_id_str) and is_on_sale = 1 order by sort desc limit 1"); 
                                    S("sql_".$md5_key,$sql_result_v,31104000);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
                        	<div class="best-about">
                    ...
                    <?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods` where  cat_id in($cat_id_str) and is_on_sale = 1 order by sort desc limit 1,6");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__goods` where  cat_id in($cat_id_str) and is_on_sale = 1 order by sort desc limit 1,6"); 
                                    S("sql_".$md5_key,$sql_result_v,31104000);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
...
<!--------底部banner-开始-------------->
	<div class="layout layer">
    	<div class="cen-banne">
            <?php $pid =4;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1495116000 and end_time > 1495116000 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(!in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
